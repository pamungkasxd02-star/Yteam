...
Observation-driven replanning may only append tools already present in the
reviewed registry. Durable approvals are bound to one job/action and consumed
once. `/cancel` takes effect at the next safe action checkpoint.
...
`/auto <target>`, `/agents`, `/approvals`, `/approve <id>`, `/deny <id>`,
`/cancel <job-id>`, and
